export class DiagramGenerator {
    generateDiagrams(analysis) {
        ...
        const dbFlows = analysis.flows.filter(f => f.destination.type === 'database' || f.source.type === 'database');
        if (dbFlows.length > 0) {
            diagrams.push(this.generateDatabaseDiagram(analysis.nodes, dbFlows));
        }
        return diagrams;
    }
    generateSystemDiagram(analysis) {
    ...
    generateCriticalPathDiagram(allNodes, criticalFlows) {
    ...
    generateDatabaseDiagram(allNodes, dbFlows) {
        ...
        return {
            ...
            nodes: allNodes.filter(n => n.type === 'database' || dbFlows.some(f => f.source.id === n.id)),
            flows: dbFlows,
            format: 'mermaid',
            diagram: mermaid
        };
    }
    inferDatabaseOperation(dataType) {
        if (dataType.includes('query') || dataType.includes('select'))
            return 'reads';
        if (dataType.includes('insert') || dataType.includes('create'))
            return 'creates';
        if (dataType.includes('update'))
            return 'updates';
        if (dataType.includes('delete'))
            return 'deletes';
        return 'accesses';
    }
    generateSequenceDiagram(flows) {
