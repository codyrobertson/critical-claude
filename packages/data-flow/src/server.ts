...
export class DataFlowServer {
  private analyzer: DataFlowAnalyzer;
  ...
  async handleToolCall(name: string, args: any): Promise<any> {
    switch (name) {
      ...
      case 'cc_data_flow_trace': {
        const { nodes, flows } = await this.flowTracer.traceRequestFlow(
          args.entryPoint,
          args.rootPath
        );
        
        const callGraph = await this.flowTracer.generateCallGraph(
          args.entryPoint,
          args.rootPath
        );
      ...
      case 'cc_data_flow_diagram': {
        ...
        if (args.diagramType !== 'all') {
          selectedDiagrams = diagrams.filter(d => 
            d.title.toLowerCase().includes(args.diagramType)
          );
        }
  ...
  private formatAnalysisReport(analysis: any): string {
    ...
    if (analysis.bottlenecks.length > 0) {
      ...
      analysis.bottlenecks.forEach((bottleneck: any) => {
        ...
        report += `   💡 Suggestions:\n`;
        bottleneck.suggestions.forEach((s: string) => {
          report += `      - ${s}\n`;
        });
        report += '\n';
      });
  ...
  private formatTraceReport(nodes: any[], flows: any[], callGraph: string): string {
  ...
  private formatDiagramsReport(diagrams: any[]): string {
