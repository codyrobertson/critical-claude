...
import { DataNode, DataFlow, DataPattern, BottleNeck, DataFlowAnalysis } from './types.js';

export class DataFlowAnalyzer {
  async analyzeDataFlow(rootPath: string): Promise<DataFlowAnalysis> {
  ...
  private async scanForDataNodes(rootPath: string, nodes: DataNode[]): Promise<void> {
    try {
      ...
      logger.error('Failed to scan for data nodes', error as Error);
    }
  }
  ...
  private identifyPatterns(flows: DataFlow[], patterns: DataPattern[]): void {
    // Check for request-response pattern
    const hasRequestResponse = flows.some(f => 
      f.protocol === 'http' && f.dataType === 'query'
    );
    ...
    const hasEvents = flows.some(f => 
      f.protocol === 'event' || f.destination.type === 'queue'
    );
  ...
  private generateRecommendations(
    ...
    if (patterns.some(p => p.pattern === 'request-response') && nodes.length > 10) {
      recommendations.push(
        'Consider introducing asynchronous patterns for better scalability'
      );
    }
    ...
    if (flows.filter(f => f.criticalPath).length > 5) {
      recommendations.push(
        'Too many critical path dependencies - consider breaking into smaller services'
      );
    }
  }
  
  private generateSummary(nodes: DataNode[], flows: DataFlow[]): string {
  ...
  private identifyDatabaseType(content: string): string {
  ...
  private detectAPIEndpoints(content: string): boolean {
    const apiPatterns = [
  ...
  private detectCacheUsage(content: string): boolean {
