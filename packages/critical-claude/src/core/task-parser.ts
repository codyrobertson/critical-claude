...
export class TaskParser {
  ...
  parse(input: string): ParsedTask {
    ...
    if (priorityMatch) {
      result.priority = priorityMatch[1].toLowerCase() as TaskPriority;
   ...
   */
  private getNextWeekday(weekdayName: string): Date {
   ...
   */
  validate(parsed: ParsedTask): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
