...
export class TemplateCommand {
  ...
  async execute(action: string, args: any[], options: any): Promise<void> {
  ...
  private async applyTemplate(templateName: string, options: any): Promise<void> {
    if (!templateName) {
  ...
  private async createTemplate(templateName: string, options: any): Promise<void> {
    if (!templateName) {
      throw new Error('Template name is required');
    }
  ...
  private async showTemplate(templateName: string, options: any): Promise<void> {
    if (!templateName) {
      throw new Error('Template name is required');
    }
  ...
  private async exportTemplate(templateName: string, options: any): Promise<void> {
    if (!templateName) {
      throw new Error('Template name is required');
    }
  ...
  private async getBuiltInTemplates(): Promise<TaskTemplate[]> {
    return [
      {
        name: 'web-app',
  ...
  private async processVariables(template: TaskTemplate, options: any): Promise<Record<string, string>> {
  ...
  private async createTasksFromTemplate(
    ...
    const taskMap = new Map<string, string>(); // template title -> created task ID
  ...
  private async createTaskFromTemplate(
    ...
    const task = await this.storage.createTask({
      title,
      description,
      priority: (templateTask.priority || 'medium') as any,
  ...
  private async selectTasksForTemplate(options: any): Promise<any[]> {
    const filter = {
  ...
  private convertTaskToTemplate(task: any): TemplateTask {
    return {
      title: task.title,
  ...
  private printTemplateTasks(tasks: TemplateTask[], indent: string = ''): void {
