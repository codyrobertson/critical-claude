...
For more help: https://critical-claude.dev/docs/tasks`;
  ...
  private async handleTemplate(input: any, options: any): Promise<void> {
  ...
  private async loadTemplate(name: string, options: any): Promise<void> {
    ...
    const content = fs.readFileSync(templatePath, 'utf8');
    let template = toml.parse(content) as any;
    ...
    const phaseTaskMap: Record<string, CommonTask[]> = {};
  ...
  private async loadTemplateData(name: string): Promise<any> {
    const templatePath = path.join(this.getTemplatesDir(), `${name}.toml`);
  ...
  private async resolveDependencies(template: any, phaseTaskMap: Record<string, CommonTask[]>): Promise<void> {
    ...
    for (const [phase, templateTasks] of Object.entries(template.tasks)) {
      if (!Array.isArray(templateTasks)) continue;
      ...
      for (let i = 0; i < templateTasks.length; i++) {
        ...
        if (!task || !taskTemplate.dependencies) continue;
