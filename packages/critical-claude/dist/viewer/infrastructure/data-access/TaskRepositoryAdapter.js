...
export class TaskRepositoryAdapter {
    ...
    async findAll(filter, sort, pagination) {
        ...
        if (filter) {
            if (filter.status && filter.status.length > 0) {
                ...
                storageFilter.status = this.mapStatusToString(filter.status[0]);
            }
            if (filter.priority && filter.priority.length > 0) {
                // Take first priority only
                storageFilter.priority = this.mapPriorityToString(filter.priority[0]);
            }
    ...
    async findByIds(ids) {
        await this.initialize();
        const tasks = await Promise.all(ids.map(id => this.storage.getTask(id.value)));
        return tasks
            .filter((t) => t !== null)
    ...
    async search(query, limit) {
        ...
        const filtered = allTasks.filter(task => task.title.toLowerCase().includes(lowerQuery) ||
    ...
    async findRootTasks() {
        ...
        const rootTasks = allTasks.filter(task => !task.labels.some(label => label.startsWith('parent:')));
    ...
    convertFromCommonTask(commonTask) {
        ...
        return new Task(new TaskId(commonTask.id), commonTask.title, commonTask.description || '', this.mapStringToStatus(commonTask.status), this.mapStringToPriority(commonTask.priority), new TaskMetadata(commonTask.assignee || 'General', commonTask.epicId || 'General', (commonTask.estimatedHours || 0) * 60, // Convert to minutes
        commonTask.actualHours ? commonTask.actualHours * 60 : undefined), new Date(commonTask.createdAt), new Date(commonTask.updatedAt), commonTask.status === 'done' ? new Date(commonTask.updatedAt) : undefined, new Set(commonTask.labels.filter(l => !l.startsWith('parent:'))), subtaskIds, parentId);
    }
