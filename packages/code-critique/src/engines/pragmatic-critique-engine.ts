...
import { SecurityAnalyzer, WebSearchTool, CodeIssue, BestPractice, LibraryIssue } from './security-analyzer.js';
...
// CodeIssue imported from security-analyzer.js
        ...
        if (issue.type === 'PERFORMANCE' && issue.fix) {
          ...
          if (bestPractices.length > 0) {
            ...
            issue.fix.description += `\n\nVerified best practices:\n${
              bestPractices.map((bp: BestPractice) => `- ${bp.practice}: ${bp.recommendation}`).join('\n')
            }`;
          }
        }
      }
