...
export class ArchitectureAnalyzer {
     ...
     */
    analyzeArchitecture(code, filename, context) {
        const issues = [];
    ...
    estimateSystemSize(code, context) {
    ...
    checkSmallSystemOverEngineering(code, overEngineering) {
        // Microservices for small apps
        if (code.includes('microservice') || code.includes('kafka') || code.includes('rabbitmq')) {
            overEngineering.push('Microservices detected for small app - Each service adds 40hrs of overhead');
        }
        // Enterprise patterns
        if (code.includes('AbstractFactory') || code.includes('interface I')) {
            overEngineering.push('Enterprise patterns in small app - YAGNI (You Ain\'t Gonna Need It)');
        }
        ...
        if (code.includes('GraphQL') && !code.includes('subscription')) {
            ...
            if (queryCount < 10) {
                overEngineering.push('GraphQL for simple CRUD - REST would be 10x faster to implement');
            }
        }
        // Kubernetes for small apps
        if (code.includes('kubernetes') || code.includes('k8s')) {
            overEngineering.push('Kubernetes for <1000 users - A $20 VPS would work fine');
        }
        // Too many layers
        ...
        const layerCount = layerIndicators.filter(layer => code.includes(layer) || code.includes(layer.toLowerCase())).length;
        if (layerCount > 3) {
            overEngineering.push(`${layerCount} layers of abstraction - This isn't enterprise Java`);
        }
    }
    checkLargeSystemUnderEngineering(code, issues) {
        // No clear architecture
        const hasArchitecture = code.includes('Controller') ||
    ...
    detectArchitecturalPatterns(code, systemSize, issues, goodPatterns) {
        // God objects
    ...
    detectAntiPatterns(code, filename, issues) {
        // Anemic domain model
