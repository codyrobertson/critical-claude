...
export class MVPPlanner {
    ...
    identifyCoreFeatures(description) {
        const features = [];
    ...
    recommendTechStack(features) {
        ...
        const needsRealtime = features.some(f => f.name.toLowerCase().includes('chat') ||
        ...
        const needsRelational = features.some(f => f.dependencies.length > 2 || f.name.includes('Analytics'));
        stack.push({
            category: 'Database',
    ...
    planPhases(features, constraints) {
        const phases = [];
    ...
    createTimeline(phases, deadline) {
        let currentWeek = 1;
    ...
    generateArchitectureDecisions(techStack, features) {
        const decisions = [];
    ...
    convertTechStackForOutput(techStack) {
        const output = {};
    ...
    calculateBudget(phases, suggestedBudget) {
        if (suggestedBudget)
...
//# sourceMappingURL=mvp-planner.js.map