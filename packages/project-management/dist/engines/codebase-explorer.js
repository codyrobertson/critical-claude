...
import { PathValidator, logger } from '@critical-claude/core';
export class CodebaseExplorer {
    ...
    async exploreCodebase(rootPath) {
        ...
        try {
            ...
            logger.error('Failed to explore codebase', { rootPath: validatedPath }, error);
            throw new Error(`Codebase exploration failed: ${error.message}`);
        }
    }
    ...
    classifyFile(filename) {
        ...
        if (this.SOURCE_EXTENSIONS.has(ext))
            return 'source';
        if (this.CONFIG_FILES.has(basename))
            return 'config';
    ...
    async createCriticalPlan(structure, issues) {
        const plan = {
