...
export class CriticalClaudeClient {
     ...
     */
    async analyzeArchitectureForTasks(rootPath) {
        try {
            ...
            logger.error('Failed to analyze architecture for tasks', error);
     ...
     */
    async analyzeDataFlowForTasks(rootPath) {
        try {
            ...
            logger.error('Failed to analyze data flow for tasks', error);
            throw new Error(`Data flow analysis failed: ${error.message}`);
        }
    }
    /**
     * Analyze sprint progress and provide insights
     */
    async analyzeSprintProgress(sprint) {
        try {
            ...
            const totalPoints = sprint.tasks
                .reduce((sum, task) => sum + task.storyPoints, 0);
            const daysElapsed = Math.floor((new Date().getTime() - sprint.startDate.getTime()) / (1000 * 60 * 60 * 24));
            const sprintLength = Math.floor((sprint.endDate.getTime() - sprint.startDate.getTime()) / (1000 * 60 * 60 * 24));
            const velocity = daysElapsed > 0 ? completedPoints / daysElapsed : 0;
            const predictedCompletion = this.calculatePredictedCompletion(sprint, completedPoints, totalPoints, velocity);
            ...
            logger.error('Failed to analyze sprint progress', error);
            throw new Error(`Sprint analysis failed: ${error.message}`);
        }
    }
    /**
     * Estimate task effort using AI
     */
    async estimateTaskEffort(taskDescription, acceptanceCriteria, codeReferences) {
        try {
            ...
            logger.error('Failed to estimate task effort', error);
            throw new Error(`Task estimation failed: ${error.message}`);
        }
    }
    // Private helper methods
    extractTasksFromPlan(planContent, originalDescription) {
        const tasks = [];
    ...
    extractTasksFromAnalysis(analysisContent, filePath) {
        const tasks = [];
    ...
    extractTasksFromArchitectureAnalysis(analysisContent) {
        const tasks = [];
    ...
    extractTasksFromDataFlowAnalysis(analysisContent) {
        const tasks = [];
    ...
    analyzeTaskComplexity(description, criteria) {
    ...
    calculatePredictedCompletion(sprint, completedPoints, totalPoints, velocity) {
    ...
    identifyRiskFactors(sprint, velocity, daysElapsed, sprintLength) {
        const risks = [];
    ...
    generateSprintSuggestions(sprint, riskFactors, velocity) {
        const suggestions = [];
    ...
    calculateConfidenceLevel(sprint, riskFactors) {
        let confidence = 0.8;
    ...
    generateMockTasks(featureDescription, context) {
