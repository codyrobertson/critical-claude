...
export class TaskStateManager {
     ...
     */
    async changeTaskState(task, newState, changedBy, reason, metadata) {
     ...
     */
    validateBasicTransition(fromState, toState) {
     ...
     */
    async validateBusinessRules(task, newState, changedBy, metadata) {
        const warnings = [];
     ...
     */
    async validateFocusTransition(task, changedBy, warnings, requiredActions) {
     ...
     */
    async validateBlockedTransition(task, metadata, warnings = [], requiredActions = []) {
        const blockerReason = metadata?.blockerReason;
     ...
     */
    async validateDoneTransition(task, warnings, requiredActions, recommendations) {
     ...
     */
    async validateInProgressTransition(task, changedBy, warnings, requiredActions) {
     ...
     */
    async validateDimmedTransition(task, warnings, requiredActions) {
     ...
     */
    async runCriticalClaudeAnalysis(task, newState) {
        try {
            ...
            logger.error('Critical Claude analysis failed', error);
            return {
                type: 'error',
                error: error.message,
     ...
     */
    async executeStateTransition(task, newState, transition, metadata) {
        const updatedTask = {
            ...task,
            status: newState,
            updatedAt: new Date(),
     ...
     */
    async executeRequiredActions(actions, task) {
