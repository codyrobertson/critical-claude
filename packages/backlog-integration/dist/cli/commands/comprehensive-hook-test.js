...
export class ComprehensiveHookTestCommand {
    ...
    async testHookFilesExist() {
        ...
        try {
            ...
            return {
                name: 'Hook files exist and are executable',
                passed: false,
                message: `Missing or non-executable hook files: ${error.message}`,
                duration: Date.now() - start,
                category: 'Basic'
            };
        }
    }
    async testClaudeCodeSettings() {
        const start = Date.now();
        try {
    ...
    async testTodoWriteHook() {
        ...
        try {
            ...
            const logContent = await fs.readFile(this.hookLogPath, 'utf-8');
            const todoWriteEntries = logContent.split('\n').filter(line => line.includes('TodoWrite') || line.includes('PreTodoWrite'));
            ...
            return {
                name: 'TodoWrite hook triggers on todo creation',
                passed: false,
                message: `TodoWrite test failed: ${error.message}`,
                duration: Date.now() - start,
                category: 'TodoWrite'
            };
        }
    }
    async testTodoReadHook() {
        const start = Date.now();
        try {
            ...
            return {
                name: 'TodoRead hook processes existing todos',
                passed: false,
                message: `TodoRead test failed: ${error.message}`,
                duration: Date.now() - start,
                category: 'TodoWrite'
            };
        }
    }
    async testBidirectionalTodoSync() {
        const start = Date.now();
        try {
            // Create a test task
    ...
    async testTodoConflictResolution() {
        const start = Date.now();
        try {
    ...
    async testWriteHook() {
        ...
        try {
            ...
            const logContent = await fs.readFile(this.hookLogPath, 'utf-8');
            const writeEntries = logContent.split('\n').filter(line => line.includes('Write') && line.includes('hook-test-file.tmp'));
            // Cleanup
            ...
            return {
                name: 'Write hook triggers on file creation',
                passed: false,
                message: `Write hook test failed: ${error.message}`,
                duration: Date.now() - start,
                category: 'FileOps'
            };
        }
    ...
    async testEditHook() {
        ...
        try {
            ...
            const logContent = await fs.readFile(this.hookLogPath, 'utf-8');
            const editEntries = logContent.split('\n').filter(line => line.includes('Edit') || line.includes('PreFileEdit'));
            return {
                name: 'Edit hook triggers on file modification',
            ...
            return {
                name: 'Edit hook triggers on file modification',
                passed: false,
                message: `Edit hook test failed: ${error.message}`,
                duration: Date.now() - start,
                category: 'FileOps'
            };
        }
    }
    async testMultiEditHook() {
        const start = Date.now();
        return {
            name: 'MultiEdit hook handles batch operations',
            passed: true,
    ...
    // MCP Tests
    async testMCPToolHooks() {
        const start = Date.now();
        try {
            const logContent = await fs.readFile(this.hookLogPath, 'utf-8');
            const mcpEntries = logContent.split('\n').filter(line => line.includes('mcp__critical-claude'));
            return {
                name: 'Critical Claude MCP tools trigger hooks',
                passed: true,
            ...
            return {
                name: 'Critical Claude MCP tools trigger hooks',
                passed: false,
                message: `MCP hook test failed: ${error.message}`,
                duration: Date.now() - start,
                category: 'MCP'
            };
        }
    }
    async testCritiqueToTasks() {
        const start = Date.now();
        return {
            name: 'Code critique creates tasks automatically',
            passed: true,
    ...
    // Agent Tests
    async testSubagentCompletion() {
        const start = Date.now();
        try {
            const logContent = await fs.readFile(this.hookLogPath, 'utf-8');
            const subagentEntries = logContent.split('\n').filter(line => line.includes('Subagent completed'));
            return {
                name: 'Subagent completion triggers cleanup',
                passed: true,
            ...
            return {
                name: 'Subagent completion triggers cleanup',
                passed: false,
                message: `Subagent test failed: ${error.message}`,
                duration: Date.now() - start,
                category: 'Agents'
            };
        }
    ...
    async testSessionContext() {
        ...
        try {
            ...
            return {
                name: 'Session context preservation works',
                passed: false,
                message: `Session context test failed: ${error.message}`,
                duration: Date.now() - start,
                category: 'Agents'
            };
        }
    ...
    async testAgentResultAggregation() {
        const start = Date.now();
        return {
            name: 'Agent result aggregation works',
            passed: true,
    ...
    // Notification Tests
    async testTaskCompletionNotification() {
        const start = Date.now();
        try {
            const logContent = await fs.readFile(this.hookLogPath, 'utf-8');
    ...
    async testSessionEndedNotification() {
        const start = Date.now();
        return {
            name: 'Session ended notifications trigger sync',
            passed: true,
    ...
    async testTaskStateSyncAccuracy() {
        const start = Date.now();
        try {
            ...
            return {
                name: 'Task state synchronization accuracy',
                passed: false,
                message: `Sync accuracy test failed: ${error.message}`,
                duration: Date.now() - start,
                category: 'Integration'
            };
        }
...
//# sourceMappingURL=comprehensive-hook-test.js.map