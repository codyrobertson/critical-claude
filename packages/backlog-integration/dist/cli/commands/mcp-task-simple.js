...
export class MCPTaskSimpleCommand {
    mcpServerPath;
    constructor() {
    ...
    async execute(action, input, options) {
        try {
            ...
            if (action === 'list' && error.message.includes('No tasks found')) {
                this.showGettingStarted();
            }
        }
    }
    async executeMCPCommand(action, input, options) {
    ...
    async listTasksFromFileSystem() {
        ...
        try {
            ...
            let output = chalk.cyan(`📋 Found ${tasks.length} tasks:\n`);
     ...
     */
    async createTaskInFileSystem(args) {
        const fs = await import('fs/promises');
        const tasksDir = path.resolve(process.cwd(), '.critical-claude', 'tasks');
        try {
            ...
            return chalk.green(`✅ Created task: ${task.title} (${taskId})`);
        }
        catch (error) {
            return chalk.red(`❌ Failed to create task: ${error.message}`);
        }
    }
    /**
     * View a specific task from file system
     */
    async viewTaskFromFileSystem(taskId) {
        if (!taskId) {
            return chalk.red('❌ Task ID is required');
        }
        const fs = await import('fs/promises');
        const tasksDir = path.resolve(process.cwd(), '.critical-claude', 'tasks');
        ...
        try {
            ...
            return chalk.red(`❌ Task not found: ${taskId}`);
        }
    }
    /**
     * Edit a task in file system
     */
    async editTaskInFileSystem(args) {
        if (!args.id) {
            return chalk.red('❌ Task ID is required');
        }
        const fs = await import('fs/promises');
        ...
        try {
            ...
            return chalk.green(`✅ Updated task: ${task.title} (${args.id})`);
        }
        catch {
            return chalk.red(`❌ Task not found: ${args.id}`);
        }
    }
    /**
     * Archive a task in file system
     */
    async archiveTaskInFileSystem(taskId) {
        if (!taskId) {
            return chalk.red('❌ Task ID is required');
        }
        const fs = await import('fs/promises');
        ...
        try {
            ...
            return chalk.green(`📦 Archived task: ${task.title} (${taskId})`);
        }
        catch {
            return chalk.red(`❌ Task not found: ${taskId}`);
        }
     ...
     */
    async createAITasksFromText(args) {
        ...
        return result + chalk.gray(`\n\n💡 Enhanced AI task generation with dependency detection and subtask creation is available via the full MCP server integration.`);
    }
    getFallbackMessage(action) {
        switch (action) {
            case 'list':
                return `📭 No tasks found. Task system is not yet initialized.

🚀 Quick Start:
  cc task create "My first task"     - Create a simple task
  cc task ai "Build login system"   - AI generates tasks from description
  
💡 The task system uses AI to break down complex requirements into manageable tasks with automatic dependency detection.`;
            case 'init':
                return `✅ Task system initialized! Ready to create AI-powered tasks.
...
Try these commands:
  ...
  cc task list`;
    ...
    showGettingStarted() {
        console.log(chalk.blue('\\n🚀 Getting Started with Critical Claude Tasks\\n'));
        ...
        console.log('');
        console.log(chalk.green('✨ Critical Claude AI will automatically:'));
        console.log('  • Break down complex requirements into tasks');
        console.log('  • Detect dependencies between tasks');
        console.log('  • Generate subtasks with proper hierarchy');
        console.log('  • Create markdown files for git integration');
    }
}
//# sourceMappingURL=mcp-task-simple.js.map