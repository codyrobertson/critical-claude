...
// EXECUTION FUNCTIONS
// ═══════════════════════════════════════════════════════════════
async function executeTaskCommand(action, args, options) {
    try {
        switch (action) {
            case 'list':
        ...
        console.error(chalk.red(`❌ Task command failed: ${error.message}`));
        process.exit(1);
    }
}
async function executeHookCommand(action, args, options) {
    try {
        const command = `cc-hooks-${action}`;
        await spawn('cc-backlog', [command], { stdio: 'inherit' });
    }
    catch (error) {
        console.error(chalk.red(`❌ Hook command failed: ${error.message}`));
        process.exit(1);
    }
}
async function executeCritiqueCommand(type, args, options) {
    try {
        if (type === 'code') {
        ...
        console.error(chalk.red(`❌ Analysis command failed: ${error.message}`));
        process.exit(1);
    }
}
async function executePlanCommand(type, args, options) {
    try {
...
async function executeUICommand(type, args, options) {
    try {
        switch (type) {
            case 'task':
                await spawn('cc-backlog', ['task-ui'], { stdio: 'inherit' });
        ...
        console.error(chalk.red(`❌ UI command failed: ${error.message}`));
        process.exit(1);
    }
}
async function executeStatusCommand(component, args, options) {
    try {
        switch (component) {
            case 'hooks':
...
//# sourceMappingURL=unified-cc-router.js.map