...
async function runClaudeWithTool(toolName: string, args: any, options: any = {}) {
...
crit
  ...
  .action(async (file, options) => {
    const context: any = {};
    if (options.users) context.user_count = options.users;
    if (options.team) context.team_size = options.team;
    if (options.problems) context.current_problems = options.problems;
...
plan
  ...
  .action(async (description, options) => {
    const context: any = {};
    if (options.team) context.teamSize = options.team;
    if (options.deadline) context.hasDeadline = true;
    if (options.stack) context.techStack = options.stack;
...
program
  ...
  .action(async (name, options) => {
    ...
    if (options.budget) constraints.budget = options.budget;
    if (options.timeline) constraints.timeline = options.timeline;
    if (options.team) constraints.teamSize = options.team;
