...
interface UIState {
  ...
  currentPath: string[];
  tree: TreeNode[];
  flatTree: TreeNode[]; // Flattened view for navigation
  tasks: EnhancedTask[];
  loading: boolean;
}

export class TaskUICommand implements CommandHandler {
  private backlogManager: BacklogManager;
  private rl!: readline.Interface; // Will be initialized in setupTerminal
  ...
  private async showRangerUI(): Promise<void> {
    this.render();
  ...
  private rebuildFlatTree(): void {
    ...
    const flattenNode = (node: TreeNode) => {
  ...
  private findNodeInTree(nodeId: string): TreeNode | undefined {
  ...
  private performSearch(): void {
    ...
    const searchInTree = (nodes: TreeNode[]) => {
      for (const node of nodes) {
  ...
  private renderDashboardHeader(width: number): void {
  ...
  private renderQuickStats(width: number): void {
  ...
  private renderTaskTable(width: number): void {
  ...
  private renderTaskDetails(width: number): void {
    const selectedTask = this.getSelectedTask();
    if (!selectedTask) {
  ...
  private renderBottomControls(width: number): void {
    console.log(chalk.gray('  ───────────────────────────────────────────────────────'));
    console.log();
  ...
  private formatTaskTableRow(task: EnhancedTask, id: number, isSelected: boolean): string {
    ...
    if (nameCol.length > 36) nameCol = nameCol.substring(0, 33) + '...';
  ...
  private getSimpleProgressBar(task: EnhancedTask): string {
  ...
  private getProgressBar(task: EnhancedTask): string {
    const progress = task.status === 'done' ? 100 : 
  ...
  private formatRangerTreeLine(node: TreeNode, isSelected: boolean, maxWidth: number): string {
    ...
    if (node.type === 'folder') {
      ...
      name = node.name;
      if (node.task) {
        if (node.task.priority === 'critical') color = chalk.red.bold;
        else if (node.task.priority === 'high') color = chalk.yellow.bold;
        else if (node.task.priority === 'medium') color = chalk.blue;
  ...
  private formatRangerDetails(task: EnhancedTask, lineIndex: number, maxWidth: number): string {
    const lines = [
      ...
      ...(task.acceptanceCriteria?.slice(0, 3).map(ac => 
        `${ac.verified ? '☑' : '☐'} ${ac.description}`) || [chalk.gray('None defined')]),
      '',
  ...
  private formatDetailsPanelLine(task: EnhancedTask, lineIndex: number, maxWidth: number): string {
    const lines = [
      `Title: ${task.title}`,
  ...
  private getRangerStatusLine(): string {
  ...
  private async editSelectedTask(): Promise<void> {
    ...
    try {
      ...
      const newPriority = await new Promise<string>((resolve) => {
      ...
      // Edit story points
      const newPointsStr = await new Promise<string>((resolve) => {
      ...
      console.log(chalk.red('  ❌ Failed to edit task: ' + (error as Error).message));
      console.log(chalk.dim('  Press any key to continue...'));
  ...
  private async createNewTask(): Promise<void> {
    ...
    try {
      ...
      console.log();
      console.log(chalk.gray('  Priority options: critical, high, medium, low'));
      const priority = await new Promise<string>((resolve) => {
      ...
      // Get story points
      const pointsStr = await new Promise<string>((resolve) => {
  ...
  private async syncTasks(): Promise<void> {
    ...
    try {
      ...
      console.log(chalk.red('❌ Sync failed: ' + (error as Error).message));
    }
