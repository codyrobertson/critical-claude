...
export class LiveCommand implements CommandHandler {
  private backlogManager: BacklogManager;
  private hookEvents: string[] = [];
  private syncEvents: string[] = [];
  private taskStats = { total: 0, pending: 0, inProgress: 0, completed: 0 };
  private lastSync = 'Never';
      ...
      tail.stdout.on('data', (data: Buffer) => {
        ...
        lines.forEach(line => {
          if (line.includes('Hook triggered')) {
            ...
            if (this.hookEvents.length > 5) this.hookEvents.pop();
      ...
      tail.stdout.on('data', (data: Buffer) => {
        ...
        lines.forEach(line => {
          if (line.includes('tasks synced')) {
            ...
            this.lastSync = time;
            if (this.syncEvents.length > 10) this.syncEvents.pop();
  ...
  private async refreshTaskStats() {
    try {
      const tasks = await this.backlogManager.listTasks();
      this.taskStats = {
        total: tasks.length,
        pending: tasks.filter(t => (t.status as string) === 'todo').length,
        inProgress: tasks.filter(t => (t.status as string) === 'in_progress').length,
        completed: tasks.filter(t => (t.status as string) === 'done').length
      };
    } catch (error) {
  ...
  private addHookEvent(event: string) {
  ...
  private addSyncEvent(event: string) {
  ...
  private updateHookActivity() {
    ...
    if (this.hookEvents.length === 0) {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        if (i < 4) process.stdout.write('\u001b[1B'); // Move down
      }
    }
  ...
  private getActivityIndicator(): string {
    const activity = [' ▫', ' ▪', ' ▫', ' ▪'];
  ...
  private getProgressBar(value: number, total: number, width: number, icon: string, color: string): string {
    if (total === 0) return `${icon} ${chalk.gray('─'.repeat(width))} 0%`.padEnd(68);
  ...
  private getSyncStatus(): string {
    ...
    if (timeSince < 5000) return '🟢';
    if (timeSince < 30000) return '🟡';
    return '🔴';
  }

  private getTimeSince(): string {
    if (this.lastSync === 'Never') return '';
    ...
    if (timeParts.length >= 3) {
      ...
      if (diffSecs < 60) return chalk.green(`(${diffSecs}s ago)`);
      if (diffSecs < 3600) return chalk.yellow(`(${Math.floor(diffSecs / 60)}m ago)`);
