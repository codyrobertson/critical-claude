...
export class HookSetupCommand implements CommandHandler {
  ...
  private async installHooks(): Promise<void> {
    ...
    try {
      ...
      const existingHook = settings.hooks.find((h: any) => h.name === 'critical-claude-sync');
      if (!existingHook) {
  ...
  private async showHookStatus(): Promise<void> {
    ...
    try {
      ...
      if (Array.isArray(hooks)) {
        ...
        const totalHooks = Object.values(hooks).reduce((sum: number, eventHooks: any) => {
          if (Array.isArray(eventHooks)) {
            return sum + eventHooks.reduce((eventSum: number, matcher: any) => {
      ...
      if (Array.isArray(hooks) && hooks.length > 0) {
        ...
        Object.entries(hooks).forEach(([eventType, matchers]: [string, any]) => {
          ...
          matchers.forEach((matcher: any, index: number) => {
            ...
            matcher.hooks?.forEach((hook: any, hookIndex: number) => {
  ...
  private async upgradeToAdvancedHooks(): Promise<void> {
