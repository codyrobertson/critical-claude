...
import { SimpleTaskManager, SimpleTask } from '../simple-task-manager.js';
import { CommandHandler } from '../command-registry.js';

export class SimpleTaskCommand implements CommandHandler {
  ...
  private async createTask(input: string[], options: any): Promise<void> {
    ...
    const priority = priorityMatch ? priorityMatch[1].toLowerCase() as any : options.priority || 'medium';
  ...
  private async listTasks(options: any): Promise<void> {
    ...
    if (options.labels) filters.labels = Array.isArray(options.labels) ? options.labels : [options.labels];
  ...
  private async updateTask(input: string[], options: any): Promise<void> {
    ...
    if (options.labels) updates.labels = Array.isArray(options.labels) ? options.labels : [options.labels];
  ...
  private async deleteTask(input: string[]): Promise<void> {
    const taskId = input[0];
    if (!taskId) {
      console.log(chalk.red('❌ Task ID required'));
      return;
  ...
  private async archiveOldTasks(): Promise<void> {
  ...
  private async showTasksByAssignee(): Promise<void> {
