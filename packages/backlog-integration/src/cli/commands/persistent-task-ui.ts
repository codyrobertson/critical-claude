...
export class PersistentTaskUICommand {
  ...
  private async syncWithClaudeCode(): Promise<void> {
    try {
  ...
  private async toggleTaskStatus(): Promise<void> {
    ...
    try {
      await this.backlogManager.changeTaskState(task.id, nextStatus as any, 'user', 'Status changed via UI');
      await this.loadTasks();
      this.render();
    } catch (error) {
  ...
  private async toggleTaskFocus(): Promise<void> {
    const task = this.state.tasks[this.state.selectedIndex];
    if (!task) return;
  ...
  private render(): void {
    if (!this.state.needsRefresh && this.state.viewMode === 'list') return;
  ...
  private async runSimpleMode(): Promise<void> {
