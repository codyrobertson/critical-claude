...
export class SimpleTaskUICommand implements CommandHandler {
  ...
  private async showTask(taskId: string | string[]): Promise<void> {
    const tasks = await this.backlogManager.listTasks();
  ...
  private getTaskStats(tasks: EnhancedTask[]) {
    return {
      total: tasks.length,
  ...
  private getStatusWithColor(status: string): string {
    switch (status) {
