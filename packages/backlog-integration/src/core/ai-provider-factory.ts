    ...
    switch (config.type) {
      case 'claude-code':
        return new EnhancedClaudeCodeProvider({
          modelId: (config.modelId as any) || 'sonnet',
...
- Following best practices`;
  ...
  private parseTasksFromResponse(content: string, featureDescription: string): AITaskSuggestion[] {
    try {
...
class AnthropicProvider implements AIProvider {
  ...
  private async getAnthropic(): Promise<any> {
    try {
      // @ts-ignore - Dynamic import at runtime
