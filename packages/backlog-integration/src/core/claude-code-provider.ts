...
export class ClaudeCodeProvider {
  ...
  async generateTasksFromFeature(
    ...
    try {
      ...
      logger.error('Failed to generate tasks via Claude Code', error as Error);
  ...
  async analyzeCodeForTasks(filePath: string): Promise<AITaskSuggestion[]> {
    ...
    try {
      ...
      logger.error('Failed to analyze code via Claude Code', error as Error);
      return [{
  ...
  private buildTaskGenerationPrompt(
    featureDescription: string,
    context: { teamSize: number; sprintLength: number; techStack?: string[] }
  ): string {
...
Generate the task breakdown now:`;
   ...
   */
  private buildCodeAnalysisPrompt(filePath: string): string {
...
Read the file and generate improvement tasks:`;
   ...
   */
  private parseTaskResponse(response: string, featureDescription: string): AITaskSuggestion[] {
    try {
      ...
      const tasks = JSON.parse(jsonMatch[0]) as Partial<AITaskSuggestion>[];
      ...
      logger.error('Failed to parse Claude Code response', error as Error);
      return this.generateFallbackTasks(featureDescription);
    }
  }
   ...
   */
  private parseCodeAnalysisResponse(response: string, filePath: string): AITaskSuggestion[] {
    try {
      const jsonMatch = response.match(/\[[\s\S]*\]/);
      if (!jsonMatch) {
        // No issues found
      ...
      logger.error('Failed to parse code analysis response', error as Error);
      return [];
    }
  }
   ...
   */
  private generateFallbackTasks(featureDescription: string): AITaskSuggestion[] {
