...
export class RealClaudeCodeIntegration {
  ...
  parseNaturalLanguage(content: string): ParsedTask {
    ...
    if (priorityMatch) {
      result.priority = priorityMatch[1].toLowerCase() as 'low' | 'medium' | 'high' | 'critical';
    }
   ...
   */
  async executeRealTodoWrite(todos: ClaudeCodeTodo[]): Promise<boolean> {
    try {
      ...
      logger.error('TodoWrite execution failed', error as Error);
      return false;
    }
  }
   ...
   */
  async executeRealTodoRead(): Promise<ClaudeCodeTodo[]> {
    try {
      ...
      logger.error('TodoRead execution failed', error as Error);
      return [];
    }
  }
   ...
   */
  private async tryDirectTodoWrite(todos: ClaudeCodeTodo[]): Promise<boolean> {
    try {
   ...
   */
  private async tryDirectTodoRead(): Promise<ClaudeCodeTodo[]> {
    try {
      // Try to execute via Claude Code if it's available in PATH
   ...
   */
  private async tryFileBasedTodoWrite(todos: ClaudeCodeTodo[]): Promise<boolean> {
    try {
      ...
      logger.error('File-based TodoWrite failed', error as Error);
      return false;
    }
  }
   ...
   */
  private async tryFileBasedTodoRead(): Promise<ClaudeCodeTodo[]> {
    try {
      const possibleFiles = [
      ...
      logger.error('File-based TodoRead failed', error as Error);
      return [];
    }
  }
   ...
   */
  async tryProcessBasedIntegration(command: string, data?: any): Promise<any> {
