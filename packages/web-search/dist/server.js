...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    switch (request.params.name) {
        case 'cc_search_vulnerabilities': {
            ...
            const libraries = request.params.arguments?.libraries || [];
            if (!code) {
            ...
            try {
                ...
                throw new Error(`Vulnerability search failed: ${error.message}`);
            }
        }
        case 'cc_verify_practices': {
            ...
            try {
                ...
                logger.error('Best practice verification failed', error);
                throw new Error(`Best practice verification failed: ${error.message}`);
            }
        }
        case 'cc_check_libraries': {
            const dependencies = request.params.arguments?.dependencies || [];
            ...
            try {
                ...
                logger.error('Library checking failed', error);
                throw new Error(`Library checking failed: ${error.message}`);
            }
        }
        case 'cc_fact_check': {
