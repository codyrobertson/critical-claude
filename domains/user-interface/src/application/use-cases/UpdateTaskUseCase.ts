  ...
  async execute(request: UpdateTaskRequest): Promise<UpdateTaskResponse> {
    ...
    try {
      ...
      if (request.status && request.status !== existingTask.status.value) {
        ...
        if (newStatus.isCompleted() && !existingTask.status.isCompleted()) {
          ...
          updatedTask = new Task(
            ...
            new Date(),
            newStatus.isCompleted() ? new Date() : updatedTask.completedAt,
            updatedTask.tags,
      ...
      if (request.title || request.description || request.tags) {
        updatedTask = new Task(
          updatedTask.id,
          request.title || updatedTask.title,
          request.description || updatedTask.description,
          updatedTask.status,
          updatedTask.priority,
          updatedTask.metadata,
          updatedTask.createdAt,
          ...
          request.tags ? new Set(request.tags) : updatedTask.tags,
          updatedTask.subtasks,
          updatedTask.parentId
        );
      }
      ...
      this.logger.error('Failed to update task', error as Error, { request });
      throw error;
    }
  }

  private getChanges(original: Task, updated: Task): Record<string, unknown> {
    ...
    if (!original.status.equals(updated.status)) changes.status = updated.status.value;
    if (!original.priority.equals(updated.priority)) changes.priority = updated.priority.value;
    if (!this.setsEqual(original.tags, updated.tags)) changes.tags = Array.from(updated.tags);
