...
export class WebSearch {
    ...
    async search(query, options = {}) {
        const { maxResults = 10, includeSnippets = true, domains = [], language = 'en' } = options;
        try {
    ...
    async searchStructured(query, options = {}) {
        try {
            if (this.apiKey) {
    ...
    async realSearch(query, options) {
        // Try SerpApi first
    ...
    async realSearchStructured(query, options) {
        // Try SerpApi first
        if (this.apiKey && this.apiKey.includes('serp')) {
    ...
    async searchWithSerpApi(query, options) {
        try {
            ...
            return this.formatSearchResults(results.map((r) => ({
                title: r.title,
                url: r.link,
                snippet: r.snippet || '',
    ...
    async searchStructuredWithSerpApi(query, options) {
        try {
            const response = await fetch(`https://serpapi.com/search.json`, {
                method: 'GET',
                headers: {
    ...
    async searchStructuredWithGoogleApi(query, options) {
        try {
            // Google Custom Search API implementation would go here
            // For now, fallback to simulation
            return this.simulateSearchStructured(query, options);
    ...
    simulateSearch(query, maxResults, includeSnippets) {
    ...
    simulateSearchStructured(query, options = {}) {
