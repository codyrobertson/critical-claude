...
import { Template, TemplateId, TemplateTask, TemplateMetadata } from '../domain/entities/Template.js';
...
export class TemplateRepository implements ITemplateRepository {
  ...
  async findByTag(tag: string): Promise<Template[]> {
    ...
    return allTemplates.filter(t => 
      t.metadata.tags && t.metadata.tags.includes(tag)
    );
  }
  ...
  async findUserCreated(): Promise<Template[]> {
    const allTemplates = await this.findAll();
  ...
  private mapToStorage(template: Template): any {
    return {
  ...
  private mapFromStorage(data: any): Template {
  ...
  private async getDefaultTemplates(): Promise<Template[]> {
    return [
      Template.create(
        'basic-project',
        'Basic project structure with common tasks',
        [
          {
            title: 'Project Setup',
        ...
        { version: '1.0', author: 'Critical Claude', tags: ['project', 'basic'] }
      ),
      ...
      Template.create(
        ...
        { version: '1.0', author: 'Critical Claude', tags: ['bug', 'fix'] }
      )
    ];
  }
}