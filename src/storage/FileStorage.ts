...
export class FileStorage {
  ...
  private async getCollectionPath(collection: string): Promise<string> {
    const basePath = await this.getBasePath();
    return path.join(basePath, `${collection}.json`);
  }

  private async getLockPath(collection: string): Promise<string> {
    const basePath = await this.getBasePath();
  ...
  private async ensureCollectionLoaded(collection: string): Promise<void> {
    if (this.loadedCollections.has(collection)) return;
  ...
  private async loadUnifiedCollection(collection: string): Promise<void> {
  ...
  private async saveCollectionToDisk(collection: string): Promise<void> {
  ...
  private async saveTasksToDirectory(): Promise<void> {
    const basePath = await this.getBasePath();
    const tasksDir = path.join(basePath, 'tasks');
    await fs.mkdir(tasksDir, { recursive: true });
  ...
  private async saveUnifiedCollection(collection: string): Promise<void> {
  ...
  async save<T extends { id: string }>(collection: string, id: string, item: T): Promise<void> {
  ...
  async find<T>(collection: string, predicate: (item: T) => boolean): Promise<T[]> {
    const allItems = await this.findAll<T>(collection);
