...
export class ProjectDetection {
  private static cache = new Map<string, ProjectInfo | null>();

  /**
   * Detect the current project from the working directory
   */
  static async detectProject(workingDir: string = process.cwd()): Promise<ProjectInfo | null> {
    // Check cache first
   ...
   */
  static async getProjectStoragePath(workingDir: string = process.cwd()): Promise<string> {
  ...
  private static async detectGitProject(workingDir: string): Promise<ProjectInfo | null> {
  ...
  private static async detectNpmProject(workingDir: string): Promise<ProjectInfo | null> {
    let currentDir = workingDir;
  ...
  private static async detectDirectoryProject(workingDir: string): Promise<ProjectInfo | null> {
  ...
  private static async saveProjectMetadata(projectInfo: ProjectInfo): Promise<void> {
    try {
      const globalStorage = path.join(os.homedir(), '.critical-claude');
