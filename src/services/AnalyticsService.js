 ...
 */
import { createMetric, calculateStats, createSuccessResult, createErrorResult } from '../models/index.js';
...
export class AnalyticsService {
    ...
    async recordMetric(data) {
        ...
        try {
            ...
            logger.silentError('Failed to record metric', error);
    ...
    async getUsageStats() {
        ...
        try {
            const metrics = await this.storage.findAll(this.COLLECTION);
            ...
            logger.silentError('Failed to get usage stats', error);
    ...
    async withTracking(command, action, operation) {
        const startTime = Date.now();
        let success = false;
        let error;
        try {
    ...
    async getMetricsByCommand(command) {
        try {
            const metrics = await this.storage.findAll(this.COLLECTION);
    ...
    async getAverageExecutionTime(command) {
        try {
            const metrics = await this.storage.findAll(this.COLLECTION);
