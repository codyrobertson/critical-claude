...
export class TemplateService {
  ...
  async listTemplates(): Promise<TemplateListResult> {
    const startTime = Date.now();
    logger.operation('Listing templates');
    
    try {
      const userTemplates = await this.storage.findAll<Template>(this.COLLECTION);
  ...
  async viewTemplate(options: { nameOrId: string }): Promise<TemplateViewResult> {
    try {
      ...
      const userTemplate = await this.storage.findById<Template>(this.COLLECTION, options.nameOrId);
  ...
  async applyTemplate(options: ApplyTemplateData): Promise<ApplyTemplateResult> {
    try {
      // Find template
      ...
      const convertTemplateTasks = (templateTasks: TemplateTask[]): void => {
  ...
  async createTemplate(data: CreateTemplateData): Promise<Result<Template>> {
    try {
      // Validate input
      if (!data.name?.trim()) {
      ...
      const existing = await this.storage.findById<Template>(this.COLLECTION, data.name);
      if (existing) {
  ...
  async deleteTemplate(nameOrId: string): Promise<Result<boolean>> {
    try {
  ...
  private processTemplateVariables(text: string, variables: Record<string, string>): string {
    return Object.entries(variables).reduce((result, [key, value]) => {
      return result.replace(new RegExp(`\\{\\{${key}\\}\\}`, 'g'), value);
    }, text);
  }

  // Utility methods removed - use core methods directly for better error handling
  // getTemplateByName: Use viewTemplate() instead
  // getUserTemplates: Use listTemplates() and filter by author
  // getBuiltInTemplateNames: Use getBuiltInTemplates() directly
}