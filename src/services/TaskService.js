 ...
 */
import { createTask, updateTask, applyFilters, createSuccessResult, createErrorResult } from '../models/index.js';
...
export class TaskService {
    ...
    async listTasks(filters = {}) {
        ...
        try {
            const allTasks = await this.storage.findAll(this.COLLECTION);
    ...
    async viewTask(id) {
        try {
            const task = await this.storage.findById(this.COLLECTION, id);
            if (!task) {
    ...
    async updateTask(id, updates) {
        ...
        try {
            ...
            const existingTask = await this.storage.findById(this.COLLECTION, id);
            if (!existingTask) {
    ...
    async importTasks(options, data) {
        try {
            let tasks;
            const errors = [];
            switch (options.format) {
            ...
            for (const taskData of tasks) {
                try {
                    ...
                    const existingTask = await this.storage.findById(this.COLLECTION, taskData.id);
                    if (existingTask && options.mergeStrategy === 'skip') {
                        continue;
                    }
                    await this.storage.save(this.COLLECTION, taskData.id, taskData);
                    imported++;
                }
                catch (error) {
