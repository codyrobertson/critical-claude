      ...
      for (const pos of positions) {
        ...
        if (hasVulnerability) {
          issues.push({
            ...
            brutal_feedback: 'SQL injection vulnerability - string concatenation in query',
      ...
      for (const pos of positions) {
        ...
        if (line.includes('${') || line.includes('+') || line.includes('user') || line.includes('input')) {
          issues.push({
            ...
            brutal_feedback: `XSS vulnerability - unsafe use of ${pattern}`,
    ...
    for (const { pattern, name, checkString } of dangerousPatterns) {
      ...
      for (const pos of positions) {
        ...
        issues.push({
          ...
          brutal_feedback: `Code execution vulnerability - ${name} is dangerous`,
